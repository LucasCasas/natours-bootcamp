...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token)
    return next(
      new AppError('You are not logged in! Reenter your credentials.')
    );
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError("Your email isn't registred!", 404));
  }
  ...
  try {
    ...
    return next(
      new AppError(
        'Something went wrong on sending recovery password email, try again later!',
        500
      )
    );
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.validPassword(req.body.passwordCurrent, user.password))) {
